package server
import ("encoding/json";"log";"net/http";"github.com/stockyard-dev/stockyard-post/internal/store")
...
s.mux.HandleFunc("GET /api/tier",func(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"tier":s.limits.Tier,"upgrade_url":"https://stockyard.dev/post/"})})
return s}
...
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{items:=s.db.List();if len(items)>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/post/");return}};var e store.Article;json.NewDecoder(r.Body).Decode(&e);if e.Title==""{we(w,400,"title required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
patch.Author=existing.Author};if patch.Slug==""{
patch.Slug=existing.Slug};if patch.Category==""{
...
func(s *Server)health(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"status":"ok","service":"post","articles":s.db.Count()})}
