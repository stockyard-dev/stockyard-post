...
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{if s.db.Count()>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/post/");return}};var e store.Article;json.NewDecoder(r.Body).Decode(&e);if e.Title==""{we(w,400,"title required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
